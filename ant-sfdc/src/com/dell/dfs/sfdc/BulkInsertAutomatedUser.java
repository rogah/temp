...
public class BulkInsertAutomatedUser extends BulkDml{
	...
	public void execute() throws BuildException {
		...
		try {
			...
			String orgId = properies.getUsername().substring(properies.getUsername().indexOf(".dfs")+4, properies.getUsername().length());

			IFileManager fileManager = new FileManager();
			
			IBulkService bulkService = new BulkService(jobManager, fileManager);

			bulkService.insertAutomatedUser(this.getFile(), orgId);
